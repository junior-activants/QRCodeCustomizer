...
var QRCode = /** @class */ (function (_super) {
    ...
    QRCode.prototype.update = function () {
        ...
        else if (qrStyle === 'extraDots') {
            ctx.fillStyle = fgColor;
            var radius = cellSize / 1.6;
            for (var row = 0; row < length; row++) {
                for (var col = 0; col < length; col++) {
                    if (qrCode.isDark(row, col) && !this.isInPositioninZone(row, col, positioningZones)) {
                        ctx.beginPath();
        ...
        else if (qrStyle === 'tetris') {
            ctx.fillStyle = fgColor;
            var cellSize = cellSize - 1; // Calculate cell size based on the length
            var gapSize = 1;
            for (var row = 0; row < length; row++) {
                for (var col = 0; col < length; col++) {
                    if (qrCode.isDark(row, col) && !this.isInPositioninZone(row, col, positioningZones)) {
                        var x = Math.round(col * (cellSize + gapSize)) + offset;
                        var y = Math.round(row * (cellSize + gapSize)) + offset;
                        ctx.fillStyle = fgColor;
        ...
        for (var i = 0; i < 3; i++) {
            ...
            if(qrStyle === 'tetris') 
                gapSize = 1;
            this.drawPositioningPattern(ctx, cellSize + gapSize, offset, row, col, color, radii);
        }
        if (logoImage) {
